...
const RGBColor_TypeDef PURPLE   = {128,0,128};
// 灯颜色缓存
...
void led_matrix_Humidity(int Humity)
{
    rt_memset(RGB_Data, 0x00, sizeof(RGB_Data));
    for(int i=0;i<Humity;i++)
    {
        if (i<5 && i>=0)
        {
            Set_LEDColor(0, BLUE);
...
// static int led_app(void)
// {
//     rt_thread_t thread;
//     thread = rt_thread_create("led",led_matrix_test4 , RT_NULL, 4096, 11, 15); 
//     if (thread == RT_NULL)
//     {
//         return -RT_ERROR;
//     }
...
//     return RT_EOK;
// }
// MSH_CMD_EXPORT(led_app,app led );